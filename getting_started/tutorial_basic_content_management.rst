...
As you can see, there are the standard page properties here, like ``URL``, ``Title``, ``META description``,
and ``META keywords``:
...
properties that are related to SEO. Position is set to 5 because we want our property editor to appear before
the ``Content`` property one (``Content`` member has position set to 10).
...
For the short properties we can set the :guilabel:`Is property visible in list` checkbox,
